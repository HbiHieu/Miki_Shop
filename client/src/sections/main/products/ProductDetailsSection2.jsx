      ...
      const showImg = () => {
         ...
         for (let i = 0 ; i < stateImageReview?.length ; i++) {
            ...
            reader.onload = () => 
            {
               ...
               if ((i === stateImageReview?.length - 1) && (stateImageReview?.length > 1)) { setStateImg(imgArr) }
               else if (stateImageReview?.length === 1) { setStateImg(imgArr) }
            }
      ...
      <>
         ...
         <div id='review' className={
            ...
            <div className='flex'>
               ...
               <p className="font-medium text-base ml-[24px] mt-[13px]">{product?.rating}.0</p>
            ...
            <div className='w-full py-[17px] px-[58px] mt-[24px] bg-[#FFFFFF]'>
               ...
               {
               stateImg[0]?.slice(5, 6) === 'i' ?
                  (stateImg?.map((img, index) => {
                     return (
                        <div className='relative mt-[6px] inline-block ml-[10px]'>
                           <IconXX DeleteFileSelect={DeleteFileSelect} indexDele={index} className='absolute top-[-8px] left-[42px]' fill='white'/>
                           <img className='w-[64px] h-[120px]' src={img} alt='anh'/>
                        </div>
                     )
                  }) )
               : 
                  (stateImg?.map((video, index) => {
                     return (
                        <div className='relative mt-[6px] inline-block ml-[10px]'>
                           <IconXX DeleteFileSelect={DeleteFileSelect} indexDele={index} className='absolute top-[-18px] left-[42px]' fill='black'/>
                           <video className='w-[64px] h-[120px]' controls>
                              <source src={video}/>
                           </video>
                        </div>
                     )
               ...
               <ul className='flex mt-[16px] items-center'>
                  <li className='text-neutral_1 font-bold text-base'>Thêm hình ảnh/video sản phẩm nếu có:</li>
                  <input
                     id='picReview'
                     className='hidden'
                     type="file"
                     multiple
                     onChange= {(e) =>
                        {setImageReview(e.target.files), setCheck(true) } 
                     } 
                  />
                  <label
                     htmlFor="picReview"
